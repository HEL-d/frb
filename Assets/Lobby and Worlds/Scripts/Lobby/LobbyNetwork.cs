...
namespace FirstGearGames.LobbyAndWorld.Lobbies
{
    public class LobbyNetwork : NetworkBehaviour
    {
        ...
        /// <summary>
        /// Called when a username is added.
        /// </summary>
        public event Action<NetworkObject> AddedUsername;
      ...
      /// <summary>
        ...
        /// <summary>
        /// Currently mumber of usernames.
        /// </summary>
public Dictionary<NetworkConnection, string> UsernameNumber = new Dictionary<NetworkConnection, string>();
        ...
        {
            _instance.AddUsernameInternal(username);
        }
        private void AddUsernameInternal(string username)
        {
            CmdAddUsername(username);
        }

        [ServerRpc(RequireOwnership = false)]
        private void CmdAddUsername(string username, NetworkConnection sender = null)
        {
            ClientInstance ci;
            if (!FindClientInstance(sender, out ci))
                return;

            UsernameNumber[ci.Owner] = username;
            ci.PlayerSettings.SetUsername(username);
            AddedUsername?.Invoke(ci.NetworkObject);
            }
